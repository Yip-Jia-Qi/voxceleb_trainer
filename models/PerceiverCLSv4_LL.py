...
class PerceiverCLS(nn.Module):
    ...
    def __init__(
        ...
        self.latentwt = nn.Linear(1,latent_dim*embed_dim)
        self.latentbd = nn.Softmax()
    ...
    def forward(self, x):
        ...
        latent_init = torch.ones((batch_size,1), device = x.device)
        latent = self.latentbd(self.latentwt(latent_init)).reshape(self.latent_dim,-1,self.embed_dim)
        # latent = latent.permute(2,0,1)
        # latent = latent.expand(-1, batch_size, -1)
